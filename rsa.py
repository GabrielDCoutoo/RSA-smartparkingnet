...
print("Após filtro de datas e câmaras:", len(df))
...
print("Após remoção de transições instáveis:", len(df))
...
with open(LOG_PATH, "w") as log:
    ...
    for camara in df['refDevice'].unique():
        ...
        for i in range(2, len(semanas_ordenadas)):
            ...
            def exportar_csv(df_input, nome_ficheiro, tipo):
                ...
                if len(df_input) >= MIN_AMOSTRAS_POR_CSV:
                    if len(df_input) > 50000:
                        df_input = df_input.sample(n=50000, random_state=42).sort_values(by="recvTime")
            ...
            # Exportações semanais (b, bb, bbb, bbbb)
...
# --- EXPORTAÇÃO CENTRALIZADO PARA TREINO CENTRALIZADO ---
inicio = df['recvTime'].min()
df['timestamp_seconds'] = (df['recvTime'] - inicio).dt.total_seconds()
df.to_csv(os.path.join(OUTPUT_DIR, "dados_centralizado.csv"), index=False)
print("✅ CSV centralizado exportado: dados_centralizado.csv")
